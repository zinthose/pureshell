...
"""
# game_example.py
# pylint: disable=line-too-long,protected-access,no-member,invalid-name,wrong-import-position
import random
...
class GameRules(Ruleset):
    ...
    def update_game_state(state: GameState) -> GameState:
        ...
        new_bullets = [
            ...
            for b in state.bullets if b.y + BULLET_HEIGHT > 0
        ]
        new_enemies = [
            replace(e, y=e.y + ENEMY_SPEED)
            for e in state.enemies if e.y < SCREEN_HEIGHT
        ]
        ...
        for i, b in enumerate(new_bullets):
            for j, e in enumerate(new_enemies):
                if GameRules._rects_collide(b.x, b.y, BULLET_WIDTH, BULLET_HEIGHT, e.x, e.y, ENEMY_WIDTH, ENEMY_HEIGHT):
                    collided_bullets.add(i)
                    collided_enemies.add(j)
                    new_score += 10

        final_bullets = [b for i, b in enumerate(new_bullets) if i not in collided_bullets]
        final_enemies = [e for i, e in enumerate(new_enemies) if i not in collided_enemies]

        is_game_over = state.is_game_over
        if not is_game_over:
            for enemy in final_enemies:
                if GameRules._rects_collide(state.player.x, state.player.y, PLAYER_WIDTH, PLAYER_HEIGHT, enemy.x, enemy.y, ENEMY_WIDTH, ENEMY_HEIGHT):
                    is_game_over = True
                    break
        ...
        return replace(state, bullets=final_bullets, enemies=final_enemies, score=new_score, is_game_over=is_game_over, spawn_timer=new_spawn_timer)
...
class Game(StatefulEntity):
    ...
    def __init__(self, screen: "Surface", clock: "Clock", font: "Font"):
        ...
        if pygame is None:
            raise ImportError("Pygame is not installed. Please install it with 'pip install pygame'")
    ...
    def update_game_state(self) -> None: raise NotImplementedError()

    @shell_method('_state', mutates=True)
    def move_player(self, direction: str) -> None: raise NotImplementedError()

    @shell_method('_state', mutates=True)
    def shoot_bullet(self) -> None: raise NotImplementedError()
    ...
    def display(self):
        ...
        player_r = pygame.Rect(self._state.player.x, self._state.player.y, PLAYER_WIDTH, PLAYER_HEIGHT)
        pygame.draw.rect(self.screen, (0, 150, 255), player_r)
        for e in self._state.enemies:
            pygame.draw.rect(self.screen, (255, 50, 50), pygame.Rect(e.x, e.y, ENEMY_WIDTH, ENEMY_HEIGHT))
        for b in self._state.bullets:
            pygame.draw.rect(self.screen, (255, 255, 100), pygame.Rect(b.x, b.y, BULLET_WIDTH, BULLET_HEIGHT))

        score_text = self.font.render(f"Score: {self._state.score}", True, (255, 255, 255))
    ...
    def run(self):
        ...
        while running:
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE: self.shoot_bullet()

            if not self._state.is_game_over:
                keys = pygame.key.get_pressed()
                if keys[pygame.K_LEFT]: self.move_player("LEFT")
                if keys[pygame.K_RIGHT]: self.move_player("RIGHT")
